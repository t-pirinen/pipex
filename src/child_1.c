...
/*   Updated: 2025/08/14 19:26:16 by tpirinen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pipex->cmd_and_params = ft_split(av[2], ' ');
...
void	child_1(struct s_pipex pipex, char **av, char **envp)
{
	...
	infile_err_check(&pipex, av);
	dup2_and_err_check(pipex.pipe[WRITE], STDOUT, &pipex);
	dup2_and_err_check(pipex.infile, STDIN, &pipex);
